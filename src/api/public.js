...
/** 终态事件（generator 收到即收尾） */
const TERMINAL_EVENTS = new Set(['received', 'expired', 'cancelled']);

/** SSE 事件构造：状态更新 */
const sseMessage = (lease) => ({ event: 'message', data: JSON.stringify(leaseView(lease)) });
/** SSE 事件构造：终态收尾（附最终状态 + 整封邮件） */
const sseDone = (lease) => ({ event: 'done', data: JSON.stringify(leaseView(lease)) });

/**
 * 租约事件流（SSE 数据源，async generator）。**先订阅再查库**：订阅早于读取，
 * 期间到达的事件必进队列，杜绝「订阅前已收码 → 漏推」（§3.6）。
...
async function* leaseEventStream(sseHub, manager, leaseId, accessCode) {
  ...
  try {
    ...
    if (!snap) return;
    yield sseMessage(snap); // 先回显当前快照（前端立即同步）
...
/**
 * 把 createLease/renew 结果（active 或 used 回看）转为统一响应。
 * @param {import('fastify').FastifyReply} reply
 * @param {object} result
 */
function sendLeaseResult(reply, result) {
  if (result.status === 'used') {
    return ok(reply, { status: 'used', results: result.results, retainUntil: result.retainUntil });
  }
  return ok(reply, leaseView(result.lease)); // 含 status:'active'
}
...
export default async function publicRoutes(app, opts) {
  ...
  // 申请邮箱（FR-6.1：用户点「申请邮箱」才调用）。分流 active / used 回看；错误 → 错误码
  app.post('/activate', { config: { rateLimit: activateRateLimit } }, async (req, reply) => {
    ...
    try {
      ...
      return sendLeaseResult(reply, result);
    } catch (err) {
  ...
  app.get('/leases/:id', async (req, reply) => {
    ...
    return ok(reply, leaseView(lease));
  });
