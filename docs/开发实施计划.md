...
| **M3** | 资源层 AccountPool | 账号注册表/健康态/原子 acquire-release/预热恢复 | §6 FR-1 | M1、M2 |
| **M4** | 收码聚合 ReceiverHub | 每账号常驻 IMAP→匹配→路由到活跃租约 + 幂等 | §6 FR-4 | M2、M3 |
...
- [ ] `src/provider/swpu/imap.js`（基座 FR-4）：`mailgate.swpu.edu.cn:993` 隐式 TLS 长连接；`subscribeMail(match,onMail)` **常驻订阅**（IDLE 监听 `exists`，断线重连）；三重匹配 + **秒级精筛**（`INTERNALDATE/Date ≥ startTime`，规避 SINCE 仅到"天"）；交付**整封邮件** MailMeta（不在此提码）
...
- [ ] `src/pool/account-pool.js`
  ...
  - [ ] `warmup()`：启动时从 Store 恢复占用关系 + 为每个启用账号建 IMAP 长连接（委托 ReceiverHub，FR-1.5）
...
**目标**：每启用账号一条常驻 IMAP，命中邮件**主动推**给该账号当前活跃租约（推模型，FR-4.1）。

**任务 WBS**
- [ ] `src/receiver/receiver-hub.js`
  - [ ] 为每账号通过 `provider.subscribeMail` 建常驻订阅（账号生命周期内持有）
...
- [ ] 重启恢复（[§3.5](#35-状态持久化与重启恢复-c-6)，C-6）：恢复占用 → 重建 IMAP → 未决 `active` 租约按 `expires_at` 立即超时巡检或续跑
...
| 关注点 | 方案 |
|---|---|
| 连接 | `imapflow` 每账号一条常驻连接（`mailgate.<domain>:993` 隐式 TLS），账号生命周期内持有 |
...
3. 重建 IMAP：为每个启用账号建常驻订阅；对持有 active 租约的账号重新绑定路由
4. 未决租约处置：
...
| IR-2 | IMAP 长连接稳定性（断连/IDLE 失效） | 漏收码/误判 | 指数退避重连 + 健康剔除（[§3.3](#33-imap-长连接与路由)）；早写 demo 长跑观察 | M2/M4 |
...
- [ ] **M4** Hub：常驻订阅 / 路由到活跃租约 / processed_mail 幂等
